...
# Finds stars which are members of the named cluster, if that cluster is included in the list. This data is from 2000.
...
# Find the dimensions of a rectangle containing all stars described in the input dataframe
...
# Returns the centre coordinates and radius of a circle containing the rectangle which contains all stars in the input dataframe.
...
# Finds the Gaia data from 2015.5 and projected 2000 data for sources in the region of a cluster - input dataframe
...
# Assign 'good_match' quality to rows with matches of less than a certain angular displacement in arcseconds whose parallaxes >0.1
...
# Takes in a dataframe and applies some maximum separation condition for a 'good_match'
...
# Takes in columns of 3D positional data on stars and gives out an array of euclidean coord.s in pc
def change_to_cartesian(ra,dec,dist,move_origin=True,plots=True):
    ...
    if move_origin==True:
        ...
        y = y - np.mean(y)
        z = z - np.mean(z)
    result = np.array(np.concatenate((x,y,z),axis=0))
...
# Takes in dataframe of stars and gives out euclidean coord.s in pc
# Optionally also returns the standard deviation of star distances (use this for 3D plot with colourmap)
def euclidean_coordinates(data,errors=False,type_errors='parsec',move_origin=True,random_sample=True,plots=False):
    mean,st_dev,p,distances = mean_st_dev(data)
    # NOT PROPAGATING RANDOM SAMPLE VAL.
    if random_sample==False:
        coord = data[['ra','dec']].values.T
    else:
        ra_data = data[['ra','ra_error']].values.T
        dec_data = data[['dec','dec_error']].values.T
        temp_ra = [np.random.normal(ra_data[0],ra_data[1])]
        temp_dec = [np.random.normal(dec_data[0],dec_data[1])]
        coord = np.concatenate((temp_ra,temp_dec))
...
# Generate a 3d plot of given cartesian coordinates - input must be an array with three rows (x,y,z) in cartesian
def three_d_plot(coord,animate=False,aspect=None,errors=int(0)):
    ...
    def init():
        ax.scatter(coord[0],coord[1],coord[2],c='b',s=5)
        ax.set_xlabel('x')
        ax.set_ylabel('y')
        ax.set_zlabel('z')
        ax.view_init(elev=10., azim=0)
        plt.title('Whitened Stellar Positions')
        return ax

    def update_view(i): 
        ax.scatter(coord[0],coord[1],coord[2],c='b',s=4)
    ...
    if animate==True:
        ...
        if errors!=int(0):
            ...
            ax.scatter(coord[0],coord[1],coord[2],c='b',s=3)
            ax.view_init(elev=10., azim=0)
...
def cluster_distances_st_dev(data_input,random_sample=True):
    ...
    plt.title('PDF of Star Position')
    plt.xlim(100,800)
    plt.plot(distances,p[0])
    plt.show()
